...
String dialogLang(
  ...
  ? _dictDialogEng[index]
  : _dictDialogEsp[index]
).map((final sel) => ins != null
  ? sel.replaceAll('*', ins)
  : sel
).run();
...
String lang(
  ...
  ? IO(() => switch(typeQuery) {
      PrintQuery.normal =>
        print(_replaceCustom(sel, custom)),
      PrintQuery.inline =>
        Console.write(_replaceCustom(sel, custom)),
      PrintQuery.warn =>
        print('${cyan('[*] ')} WARNING: ${_replaceCustom(sel, custom)}'),
      PrintQuery.error =>
        print('${red('[*] ')} ERROR: ${_replaceCustom(sel, custom)}')
    })
    .map((final _) => '').run()
